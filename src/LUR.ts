...
class lurValue {
	...
	/** 上一个元素 */
	prev: lurValue | undefined;

	/** 下一个元素 */
	next: lurValue | undefined;
...
export default class LRU {
	/** 缓存的键 */
	datas = new Map<string, lurValue>();

	/** 最大缓存数量 */
	capacity = 100;

	/** 头部元素 */
	private head: lurValue | undefined;

	/** 尾部元素 */
	private tail: lurValue | undefined;
	...
	set(key: string, value: any, exp: number) {
		...
		if (item) {
			...
			// 直接添加
			item = new lurValue(key, value, exp);
	...
	/** 更新调整位置，将新数据移动到尾部 */
	private _updateItem(item: lurValue) {
