...
function exportJson(data, fileName, title, filter) {
  var _a;
  data = _chunkXDQMWDHBcjs.toJSON.call(void 0, data);
  if (!_chunkXDQMWDHBcjs.hasArray.call(void 0, data)) return;
  const excel = ["<table>"];
...
function exportTable(tableHtml, fileName, title) {
  if (!tableHtml || !_chunkXDQMWDHBcjs.isString.call(void 0, tableHtml)) return;
...
var jsonDownload = (json, fileName = "\u7ED3\u679C") => {
  if (!json) return;
  objectDownload(_chunkXDQMWDHBcjs.toJSON.call(void 0, json), fileName);
};
...
async function QRObject(params) {
  if (!params || !_chunkXDQMWDHBcjs.hasObject.call(void 0, params) || !params.code) return;
...
async function remoteFileToBase64(url, onlyImage = true) {
  return new Promise(async (resolve, reject) => {
    await fetch(url).then((res) => res.blob()).then((blob) => {
      ...
      reader.onload = () => {
        if (_chunkXDQMWDHBcjs.hasString.call(void 0, reader.result)) {
...
function setCookie(name, value, options = {}) {
  if (_chunkXDQMWDHBcjs.SERVERMODE) return;
  try {
    let expires = "";
...
function getCookie(name) {
  if (_chunkXDQMWDHBcjs.SERVERMODE) return null;
  try {
    const nameEQ = name + "=";
...
var FormValidate = class {
  ...
  concat(...args) {
    ...
    for (let index = 0; index < args.length; index++) {
      ...
      _chunkXDQMWDHBcjs.hasArray.call(void 0, rule) && (rules = _chunkXDQMWDHBcjs.merge.call(void 0, rules, rule));
    }
    return rules;
  }
  /********************************************************************/
  ...
  validate(rules, value) {
    ...
    if (!_chunkXDQMWDHBcjs.hasArray.call(void 0, rules)) return true;
    const messages = rules.filter((rule) => _chunkXDQMWDHBcjs.hasObject.call(void 0, rule)).map((rule) => this.validateRule(rule, value)).filter((flag) => flag !== true);
  ...
  validateRequired(rule, value) {
    ...
    if (_chunkXDQMWDHBcjs.isNil.call(void 0, value)) return message;
    if (_chunkXDQMWDHBcjs.isArray.call(void 0, value)) return value.length > 0 ? true : message;
    if (_chunkXDQMWDHBcjs.isObject.call(void 0, value)) return Object.keys(value).length > 0 ? true : message;
    if (_chunkXDQMWDHBcjs.isString.call(void 0, value)) {
      if (rule.trim === true) value = value.trim();
      return value.length > 0 ? true : message;
    }
    if (_chunkXDQMWDHBcjs.isBoolean.call(void 0, value)) return value === true ? true : message;
    return true;
  }
  /** 验证正则表达式，仅针对字符串 */
  validateRegular(rule, value) {
    if (!value || !rule || !rule.pattern) return true;
    if (!_chunkXDQMWDHBcjs.isString.call(void 0, value)) return "\u975E\u6709\u6548\u6587\u672C\uFF0C\u4E0D\u80FD\u8FDB\u884C\u6B63\u5219\u6BD4\u8F83";
  ...
  validateType(rule, value) {
    ...
    if (rule.type === "number") return _chunkXDQMWDHBcjs.isNumber.call(void 0, value) ? true : message("\u6570\u5B57");
    if (!_chunkXDQMWDHBcjs.isString.call(void 0, value) && !_chunkXDQMWDHBcjs.isNumber.call(void 0, value)) return message("\u7C7B\u578B\uFF0C\u4EC5\u652F\u6301\u5224\u65AD\u5B57\u7B26\u6216\u6570\u5B57\u683C\u5F0F");
    value = value.toString();
    if (rule.type === "json") return _chunkXDQMWDHBcjs.isJSON.call(void 0, value) ? true : message("JSON");
    if (rule.type === "url") return _chunkXDQMWDHBcjs.isHttp.call(void 0, value) ? true : message("\u7F51\u5740");
    if (rule.type === "email") return _chunkXDQMWDHBcjs.isEmail.call(void 0, value) ? true : message("\u90AE\u7BB1");
    if (rule.type === "tel") return _chunkXDQMWDHBcjs.isPhone.call(void 0, value) ? true : message("\u7535\u8BDD\u53F7\u7801");
    if (rule.type === "phone") return _chunkXDQMWDHBcjs.isPhone.call(void 0, value) ? true : message("\u7535\u8BDD\u53F7\u7801");
    if (rule.type === "mobile") return _chunkXDQMWDHBcjs.isMobile.call(void 0, value) ? true : message("\u624B\u673A\u53F7\u7801");
    if (rule.type === "mobilephone") return _chunkXDQMWDHBcjs.isMobile.call(void 0, value) ? true : message("\u624B\u673A\u53F7\u7801");
    if (rule.type === "guid") return _chunkXDQMWDHBcjs.isGuid.call(void 0, value) ? true : message("GUID");
    if (rule.type === "chinese") return _chunkXDQMWDHBcjs.isChinese.call(void 0, value) ? true : message("\u4E2D\u6587\u5B57\u7B26");
    if (rule.type === "english") return _chunkXDQMWDHBcjs.isEnglish.call(void 0, value) ? true : message("\u82F1\u6587\u5B57\u7B26");
    if (rule.type === "ip") return _chunkXDQMWDHBcjs.isIP.call(void 0, value) ? true : message("IP");
    if (rule.type === "name")
      return _chunkXDQMWDHBcjs.isName.call(void 0, value) ? true : "\u4EC5\u652F\u6301\u5B57\u6BCD\u6570\u5B57\u6A2A\u7EBF\u5C0F\u6570\u70B9\uFF0C\u4E14\u9700\u5B57\u6BCD\u5F00\u5934";
    return "\u4E0D\u652F\u6301\u6B64\u683C\u5F0F [" + rule.type + "] \u9A8C\u8BC1";
  }
  /** 验证区间 */
  validateRange(rule, value) {
    if (_chunkXDQMWDHBcjs.isNil.call(void 0, value) || !rule) return true;
    if (rule.min) {
      const min = Number(rule.min);
      if (!isNaN(min)) {
        if (_chunkXDQMWDHBcjs.isString.call(void 0, value)) value = Number(value);
        if (isNaN(value)) return "\u6B64\u503C\u5FC5\u987B\u4E3A\u6570\u503C";
        if (_chunkXDQMWDHBcjs.isNumber.call(void 0, value) && value < min)
    ...
    if (rule.max) {
      ...
      if (!isNaN(max)) {
        if (_chunkXDQMWDHBcjs.isString.call(void 0, value)) value = Number(value);
        if (isNaN(value)) return "\u6B64\u503C\u5FC5\u987B\u4E3A\u6570\u503C";
        if (_chunkXDQMWDHBcjs.isNumber.call(void 0, value) && value > max)
    ...
    if (rule.minLength) {
      ...
      if (!isNaN(min)) {
        if (_chunkXDQMWDHBcjs.isNumber.call(void 0, value)) value = value.toString();
        if ((_chunkXDQMWDHBcjs.isString.call(void 0, value) || _chunkXDQMWDHBcjs.isArray.call(void 0, value)) && value.length < min)
    ...
    if (rule.maxLength) {
      ...
      if (!isNaN(max)) {
        if (_chunkXDQMWDHBcjs.isNumber.call(void 0, value)) value = value.toString();
        if ((_chunkXDQMWDHBcjs.isString.call(void 0, value) || _chunkXDQMWDHBcjs.isArray.call(void 0, value)) && value.length > max)
  ...
  validateFunction(rule, value) {
    if (!rule || !_chunkXDQMWDHBcjs.isFn.call(void 0, rule.validate)) return true;
  ...
  description(rules) {
    const ret = [];
    rules = this.updateRules(rules);
    if (!_chunkXDQMWDHBcjs.hasArray.call(void 0, rules)) return "";
    rules.filter((rule) => _chunkXDQMWDHBcjs.hasObject.call(void 0, rule)).forEach((rule) => {
  ...
  descriptionType(rule) {
    if (!rule || !rule.type || !_chunkXDQMWDHBcjs.isString.call(void 0, rule.type)) return "";
  ...
  /** 是否存在规则 */
  hasRules(rules) {
    return _chunkXDQMWDHBcjs.hasArray.call(void 0, this.updateRules(rules));
  }
  /** 将规则对象转换成规则列表 */
  updateRules(rules) {
    if (_chunkXDQMWDHBcjs.hasArray.call(void 0, rules)) return rules;
    if (!_chunkXDQMWDHBcjs.hasObject.call(void 0, rules)) return [];
    const rs = rules;
...
var waterMark_default = (background, interval = 5) => {
  if (_chunkXDQMWDHBcjs.SERVERMODE || !background) return;
  const validate = () => {
    const img = _chunkXDQMWDHBcjs.isFn.call(void 0, background) ? background() : background;
    if (!img) return;
...
function UIThemeQuery(options) {
  if (!_chunkXDQMWDHBcjs.SERVERMODE) {
    ...
    !_chunkXDQMWDHBcjs.hasArray.call(void 0, defaultThemes) && (defaultThemes = []);
...
var createImportantStyle = (days) => {
  if (!days || _chunkXDQMWDHBcjs.SERVERMODE) return;
  if (_chunkXDQMWDHBcjs.isString.call(void 0, days)) days = days.split(",");
  if (_chunkXDQMWDHBcjs.hasArray.call(void 0, days)) {
    const list = {};
    days.forEach((day2) => {
      day2 = day2.trim();
      day2 && (list[day2] = "");
  ...
  if (!_chunkXDQMWDHBcjs.hasObject.call(void 0, days)) return;
  ...
  let info = days[day];
  let background = "";
  if (_chunkXDQMWDHBcjs.isString.call(void 0, info) && info !== "") {
    if (info.startsWith("*")) {
      info = info.slice(1);
...
var mergeClass = (...classNames) => {
  ...
  for (let className of classNames) {
    if (_chunkXDQMWDHBcjs.isFn.call(void 0, className)) {
      className = className();
    }
    if (_chunkXDQMWDHBcjs.isString.call(void 0, className)) {
      result.push(className);
    } else if (_chunkXDQMWDHBcjs.isArray.call(void 0, className)) {
      result.push(...className);
    } else if (_chunkXDQMWDHBcjs.isObject.call(void 0, className)) {
  ...
  return _chunkXDQMWDHBcjs.cleanDuplicate.call(void 0, result);
};

// src/page.ts
var screenType = (options) => {
  if (_chunkXDQMWDHBcjs.SERVERMODE) return "server";
...
var isFullscreen = () => {
  if (_chunkXDQMWDHBcjs.SERVERMODE) return false;
  return (
    // @ts-ignore
...
var _elementAction = (element, elementFunctionName) => {
  try {
    ...
    if (_chunkXDQMWDHBcjs.isFn.call(void 0, fun)) {
      fun();
      return true;
    }
  } catch (e2) {
...
var fullscreenLaunch = (element) => {
  if (_chunkXDQMWDHBcjs.SERVERMODE) return false;
  let ele = _chunkXDQMWDHBcjs.isString.call(void 0, element) ? document.querySelector(element) : element;
