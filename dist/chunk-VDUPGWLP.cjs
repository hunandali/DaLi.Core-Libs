...
var ConsoleEcho = class {
  ...
  _echo(icon, bgColor, color, mode, message, ...optionalParams) {
    if ((!_chunkU4AQ4RPTcjs.DEBUG || _chunkU4AQ4RPTcjs.TEST) && mode !== "error") return;
    const stringify = (objColor, obj) => {
      obj = emptyString(obj);
      const str = _chunkU4AQ4RPTcjs.isObject.call(void 0, obj) ? JSON.stringify(obj, null, "	") : obj;
      return objColor(` ${str} `);
    };
    ...
    let source = _chalk2.default.reset.white(_chunkU4AQ4RPTcjs.errorTrace.call(void 0, 1, 1, ["$Global.echo", "consoleEcho."]));
    if (_chunkU4AQ4RPTcjs.hasString.call(void 0, message) && _chunkU4AQ4RPTcjs.hasArray.call(void 0, optionalParams)) {
      for (let i = 0; i < optionalParams.length; i++) {
        if (!_chunkU4AQ4RPTcjs.hasObject.call(void 0, optionalParams[i])) continue;
        if (message.includes("{") && message.includes("}")) break;
        message = _chunkU4AQ4RPTcjs.template.call(void 0, message, optionalParams[i]);
      }
    }
    if (message) {
    ...
    optionalParams.forEach((par, index) => {
      if (index === 0 && _chunkU4AQ4RPTcjs.isFn.call(void 0, par)) {
        par();
      } else {
        if (showIndex) {
  ...
  log(message, ...optionalParams) {
    _chunkU4AQ4RPTcjs.DEBUG && console.log(message, ...optionalParams);
  }
  /** 表格显示 */
  ...
  debug(...optionalParams) {
    var _a, _b;
    if (!_chunkU4AQ4RPTcjs.hasArray.call(void 0, optionalParams)) return;
    const enabled = optionalParams[optionalParams.length - 1];
    if (enabled === "~") return;
    let title = "";
    let id = _chalk2.default.bgGray.yellowBright(` ${_chunkU4AQ4RPTcjs.globalId.call(void 0, )} `);
    if (_chunkU4AQ4RPTcjs.isString.call(void 0, optionalParams[0]) && optionalParams[0]) {
    ...
    if (!_chunkU4AQ4RPTcjs.SERVERMODE) {
      ...
      if (_chunkU4AQ4RPTcjs.hasArray.call(void 0, infos)) {
